...
function fmt(s: number) { if(!isFinite(s)||s<0) return '0:00'; return `${String(Math.floor(s/60)).padStart(2,'0')}:${String(Math.floor(s%60)).padStart(2,'0')}` }
...
export default function AudioPlayer({ messageId, telefone, fromMe=false, isOwn }: Props) {
  ...
  useEffect(() => {
    if(b64 && !blobUrl){ const url=toBlobUrl(b64); blobRef.current=url; setBlobUrl(url) }
    return () => { if(blobRef.current) URL.revokeObjectURL(blobRef.current) }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [b64])

  if(!messageId) return <div className="flex items-center gap-1.5 text-sm opacity-60 py-0.5">🎵 <span>Não indexado</span></div>
  ...
  const o = isOwn
    ? { btn:'bg-white/20 hover:bg-white/30 text-white', track:'bg-white/20', fill:'bg-white/80', time:'text-white/70' }
    : { btn:'bg-gray-100 hover:bg-gray-200 text-gray-700', track:'bg-gray-200', fill:'bg-primary', time:'text-gray-400' }
    ...
    <div className="flex items-center gap-2.5 w-52">
      {blobUrl && (
        <audio ref={audioRef} src={blobUrl} preload="auto"
          ...
          onEnded={()=>{ setPlaying(false); setCur(0); if(audioRef.current) audioRef.current.currentTime=0 }}/>
      )}
      <button onClick={()=>blobUrl?(playing?audioRef.current?.pause():audioRef.current?.play()):null}
        className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 transition-colors ${o.btn}`}>
        {loading ? <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin"/>
          : playing ? <Pause size={14}/> : <Play size={14} className="translate-x-0.5"/>}
      </button>
      <div className="flex-1 flex flex-col gap-1">
        <div className={`relative h-1.5 rounded-full cursor-pointer ${o.track}`}
          ...
          <div className={`absolute left-0 top-0 h-full rounded-full ${o.fill}`} style={{width:`${prog*100}%`}}/>
        </div>
        <div className="flex justify-between">
          <span className={`text-[10px] tabular-nums ${o.time}`}>{fmt(cur)}</span>
          <span className={`text-[10px] tabular-nums ${o.time}`}>{blobUrl?fmt(dur):'—'}</span>
        </div>
      </div>
    </div>
  )
