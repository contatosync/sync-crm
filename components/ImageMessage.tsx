...
export default function ImageMessage({ messageId, telefone, fromMe=false, localDataUrl }: Props) {
  ...
  if(!messageId && !localDataUrl) return <div className="flex items-center gap-1.5 text-sm opacity-60 py-1">🖼️ <span>Não disponível</span></div>
  ...
  return (
    <>
      <img src={src} alt="Imagem" onClick={()=>setLb(true)}
        className="rounded-lg max-w-[200px] max-h-[200px] object-cover cursor-pointer hover:opacity-90 transition-opacity"/>
      {lb && (
        <div className="fixed inset-0 bg-black/85 flex items-center justify-center z-[200]" onClick={()=>setLb(false)}>
          <button className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-white" onClick={()=>setLb(false)}><X size={20}/></button>
